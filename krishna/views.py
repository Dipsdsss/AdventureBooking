...
def payment(request,):
    ...
    return HttpResponse(render(request, 'payment.html', {'room': room, 'price': price}))
    # room = reservation.room
...
def edit_room(request):
    ...
    if request.method == 'POST' and request.user.is_staff:
        ...
        old_room.adventure_type = request.POST['type']
...
def add_new_room(request):
    ...
    if request.method == "POST":
        ...
        new_room.adventure_type = request.POST['type']
        ...
        new_room.save()
        messages.success(request,"New Adventure Added Successfully")
    ...
    if request.method =="POST":
        ...
        for each_reservation in Reservation.objects.all().filter(room = room):
            if str(each_reservation.check_in) < str(request.POST['check_in']) and str(each_reservation.check_out) < str(request.POST['check_out']):
                ...
                messages.warning(request,"Sorry, It is already Booked")
                return redirect("homepage")
